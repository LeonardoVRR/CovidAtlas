...
const WorldMap = ({ clickCountry }) => {
  ...
  function zoomMap(event) {
    ...
    newScale = Math.min(3, Math.max(0.5, newScale)); // Limites de zoom
    setScale(newScale);
  }

  function zoomMapButton(zoom, scale) {
    if (zoom != "reset") {
      let newScale = scale;

      if (zoom === "in") {
        newScale = scale + 0.1; // Zoom in aumenta a escala
      } else if (zoom === "out") {
        newScale = scale - 0.1; // Zoom out diminui a escala
      }

      // Limita a escala para estar entre 0.5 e 3
  ...
  function holdMap(event) {
    ...
    // Calcula o deslocamento inicial do clique dentro do SVG
    offset.current = {
      x: (event.clientX - containerRect.left - positionMap.left) / scale,
      y: (event.clientY - containerRect.top - positionMap.top) / scale,
    };

    isDragging.current = true;

    // Adiciona eventos globais para garantir que funcione mesmo se sair do contêiner
    document.addEventListener("mousemove", moveMap);
    document.addEventListener("mouseup", dropMap);
  }
  ...
  function moveMap(event) {
    ...
    if (event.currentTarget.id !== "mapContainer") return;

    const containerRect = svgContainerRef.current.getBoundingClientRect();

    setPositionMap({
      left: event.clientX - containerRect.left - offset.current.x * scale,
      top: event.clientY - containerRect.top - offset.current.y * scale,
    });
  }
  ...
  function dropMap() {
    ...
    svgContainerRef.current.style.cursor = "auto";
    document.removeEventListener("mousemove", moveMap);
    document.removeEventListener("mouseup", dropMap);
  }

  // function markSelectedCountry(event) {
  //   if (event.target.tagName == "path") {
  ...
  //   holdMap(event);
  //   markSelectedCountry(event);
  // };

  return (
    <div className="w-full h-full overflow-hidden relative border-1 border-solid border-black">
      <div
        id="zoomContainer"
        className="bg-[#222222] text-white text-center absolute right-1 bottom-1 flex flex-col cursor-pointer z-[60]"
      >
        <button
          title="Reset"
          onClick={() => {
            zoomMapButton("reset", scale);
          }}
        >
      ...
      <div
        id="mapContainer"
        ref={svgContainerRef}
        className="w-full h-full overflow-hidden relative bg-[#000d17]"
        onMouseMove={moveMap}
        onMouseUp={dropMap}
      >
        {/* <MapMaker
      ...
      /> */}
        <svg
          ...
          className="w-full h-full pr-4 absolute cursor-grab"
          style={{
            left: `${positionMap.left}px`,
            top: `${positionMap.top}px`,
          ...
          onMouseDown={holdMap}
        >
          <g className="fill-[#2a2a28] cursor-pointer relative">
            <path
            ...
            <path
              ...
              id="BN"
              data-countryname="Brunei Darussalam"
            ></path>
            <path
            ...
            <path
              ...
              id="MK"
              data-countryname="North Macedonia"
            ></path>
            <path
            ...
            <path
              ...
              id="BL"
              data-countryname="Saint-Barthélemy"
            ></path>
            <path
            ...
            ></path>
            <path
              data-countryname="Faeroe Islands"
            ...
            ></path>
            <path
              data-countryname="Federated States of Micronesia"
            ...
            ></path>
            <path
              data-countryname="São Tomé and Principe"
            ...
            <path
              ...
              id="VG"
              data-countryname="British Virgin Islands"
            ></path>
            <path
              data-countryname="United States Virgin Islands"
            ...
            <path
              ...
              id="BQSE"
              data-countryname="St. Eustatius (Netherlands)"
            ></path>
            <path
              d="M632.8 388.7l-0.1 0 0.1-0.2 0.1 0.1-0.1 0.1z"
              id="BQSA"
              data-countryname="Saba (Netherlands)"
            ></path>
            <path
